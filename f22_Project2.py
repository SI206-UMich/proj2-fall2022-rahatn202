...
.
    ...
    if num_bed == "Studio":
        num_bed = "1"
    return (policy_name, room_type, int(num_bed))
...
def get_detailed_listing_database(html_file):
    ...
    for listing in listings:
        ...
        lsting_database.append(listing + listing_info)
    return lsting_database
...
def check_policy_numbers(data):
    ...
    """
    policy_lst = []
    reg_exp = r"20\d{2}-00\d{4}STR|STR-000\d{4}"
    for item in data:
        
        x = re.search(reg_exp, item[3])
        if not x:
...
def extra_credit(listing_id):
    ...
    """
    file_html = f"html_files/listing_{listing_id}_reviews.html"
    with open(file_html, encoding='utf8') as f:
        soup = BeautifulSoup(f, 'html.parser')
        review_date_lst = {}
        #review dates are as list and have class name of "_1f1oir5"
        tag_lst = soup.find_all("li", class_="_1f1oir5")
        for tag in tag_lst:
            #item on index 1 is year, so will create a key with years by spliting text of the tag li
            key = tag.text.split()[1]
            #creating review date dictionary with year as key and num of times it appears as value
            if key in review_date_lst:
                review_date_lst[key] += 1
            else:
                review_date_lst[key] = 1
                #checking if value of each year is more than 90 times by looping through values list
        for value in review_date_lst.values():
            if value > 90:
                return False
            else:
                return True
